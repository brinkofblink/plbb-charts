...
const charts = tisTheSeason
  ? AllCharts
  : AllCharts.filter((c) => c.categories.includes('christmas') === false);
...
const createResultElement = (chart) => {
  ...
  if (chart.key) {
    const chartKey = document.createElement('span');
    chartKey.setAttribute('class', 'chart-key');
    const chartKeyText = document.createTextNode(chart.key);
    chartKey.appendChild(chartKeyText);
    chartTitle.appendChild(chartKey);
  }
...
if (tisTheSeason) {
  document.documentElement.style.setProperty(
    '--bg',
    'linear-gradient(0deg, #e3a97c, #a5b6ca, #c6d5e7, #c2c3d8)',
  );
  document.documentElement.style.setProperty('--color-light', '#a1afca');
  document
    .querySelector('main')
    .appendChild(document.createElement('snow-fall'));
}
